<template>
    ...
    <button v-if="isOpen" class="hambuger self-end" @click="closeMenu">
        <img src="/src/assets/images/icon-menu-close.svg" alt="close" />
    </button>

    <transition>
        <div v-if="isOpen" class="bg-black/50 fixed h-screen w-screen top-0 left-0">
            <div class="flex flex-col p-4 w-8/12 ml-auto bg-app-almost-white h-full">
                <div class="h-full max-h-16"></div>
